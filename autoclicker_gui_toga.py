...
class RobloxAutoclickerApp(toga.App):
    def startup(self):
        ...
        footer_label = toga.Label(
            ...
            style=Pack(padding=(5, 5), font_size=9, color="#64748b")
        )
    ...
    def create_control_tab(self):
        ...
        box = toga.Box(style=Pack(direction=COLUMN, padding=(20, 20)))

        # Status
        status_box = toga.Box(style=Pack(direction=ROW, padding=(10, 10)))
        ...
        self.config_info = toga.Label(
            "",
            style=Pack(font_size=10, padding=(0, 10))
        )
        ...
        # Buttons
        button_box = toga.Box(style=Pack(direction=ROW, padding=(10, 10)))
        ...
        log_label = toga.Label(
            "📋 Live Logs:",
            style=Pack(padding=(10, 10), font_weight="bold")
        )
        box.add(log_label)

        self.log_text = toga.MultilineTextInput(
            readonly=True,
            style=Pack(flex=1, padding=(10, 10))
        )
    ...
    def create_config_tab(self):
        ...
        # CPS
        cps_label = toga.Label("Klicks pro Sekunde (CPS):", style=Pack(padding=(5, 5)))
        box.add(cps_label)
        ...
        # Hotkey
        hotkey_label = toga.Label("Aktivierungs-Hotkey:", style=Pack(padding=(5, 5)))
        ...
        # Aktivierungsmodus
        mode_label = toga.Label("Aktivierungs-Modus:", style=Pack(padding=(5, 5)))
        box.add(mode_label)
    ...
    def create_test_tab(self):
        ...
        title = toga.Label(
            "🎯 Klick-Test Bereich",
            style=Pack(padding=(10, 10), font_size=16, font_weight="bold")
        )
        box.add(title)

        info = toga.Label(
            "Klicke auf den Button unten, um deine Clicking-Geschwindigkeit zu testen",
            style=Pack(padding=(5, 5), font_size=10)
        )
        box.add(info)
        ...
        # Stats
        stats_box = toga.Box(style=Pack(direction=ROW, padding=(10, 10)))

        self.current_cps_label = toga.Label("Aktuelle CPS: 0", style=Pack(padding=(5, 5)))
        stats_box.add(self.current_cps_label)

        self.avg_cps_label = toga.Label("Durchschnitt: 0", style=Pack(padding=(5, 5)))
        stats_box.add(self.avg_cps_label)

        box.add(stats_box)

        stats_box2 = toga.Box(style=Pack(direction=ROW, padding=(10, 10)))

        self.total_clicks_label = toga.Label("Gesamt: 0", style=Pack(padding=(5, 5)))
        stats_box2.add(self.total_clicks_label)

        self.duration_label = toga.Label("Dauer: 0s", style=Pack(padding=(5, 5)))
    ...
    def save_config(self, widget):
        ...
        try:
            ...
            self.main_window.info_dialog("Erfolg", "✅ Konfiguration erfolgreich gespeichert!")
            ...
            self.main_window.error_dialog("Fehler", f"Fehler beim Speichern:\n{e}")
    ...
    def start_autoclicker(self, widget):
        ...
        try:
            ...
            self.main_window.error_dialog("Fehler", f"Fehler beim Starten:\n{e}")
